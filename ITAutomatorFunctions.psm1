...
if ((Test-Path("$scriptDir\ITAutomatorFunctions.psm1"))) {Import-Module "$scriptDir\ITAutomatorFunctions.psm1" -Force} else {write-host "Err: Couldn't find ITAutomatorFunctions.psm1";return}
...
Function Get-CredentialInFile
{
    ...
    ## "
    ## ##
    ## Write-Host "Sending to $($Globals.email_to)"
    ## $smtp.Send($msg)
    ## ########
...
# Only copies files that need copying 
# if comparemethod=hash , based on hash (hash computation my take a long time for large files)
# if comparemethod=date , based on date,size
# Returns a list of files with status of each (an array of strings)
# Usage: 
...

{
    ...
    { # Target folder exists
        ...
        { # Each file
            ...
            #############
            #$source
            #C:\Users\Jason\ITAutomator\Staff - General\Programming.Powershell\MSOffice Templates\MS Office Templates\Office2016_Themes
            #$file.FullName
            #C:\Users\Jason\ITAutomator\Staff - General\Programming.Powershell\MSOffice Templates\MS Office Templates\Office2016_Themes\Truvvo.thmx
            #$target
            #C:\Users\Jason\AppData\Roaming\Microsoft\Templates\Document Themes
            #$target_path
            #C:\Users\Jason\AppData\Roaming\Microsoft\Templates\Document Themes\Truvvo.thmx
            #
...
Function  UpdateXML
{
    ...
    ###
    $xmlNode = $nppXML.SelectNodes($xmlPath) | Where {$_.name -eq $xmlAttr}
    if (!($xmlNode))
    { #no node
...
Function LoadModule ($m, $providercheck = "") #nuget
{
<# Example:
...
#>
    ...
    # First see what the latest version online is
    ...
    If ($minfo) {
        ...
        if ($mod_avail)
        {
            ...
            {
                $strReturn +="OK:GET v$($minfo[0].Version.ToString()) [Current version]"
            }
            else
            {
                $strReturn +="OK:GET v$($minfo[0].Version.ToString()) [Update available to v$($mod_avail[0].Version.ToString()) use Update-Module (as admin)]"
            }
            }
        else
        {
            $strReturn +="OK:GET v$($minfo[0].Version.ToString())"
        }
    }
    Else {
        ...
        if ($minfo) {
            ...
            Import-Module $m
            $strReturn +="OK:Imported v$($minfo[0].Version.ToString())"
            if ($mod_avail)
            {
                if ($minfo[0].Version.ToString() -eq $mod_avail[0].Version.ToString())
                {
                    $strReturn +="OK:Imported v$($minfo[0].Version.ToString()) [Current version]"
                }
                else
                {
                    $strReturn +="OK:Imported v$($minfo[0].Version.ToString()) [Update available to v$($mod_avail[0].Version.ToString()) use Update-Module (as admin)]"
                }
                }
            else
            {
...
Function GetWhoisData ($domain, $cache_hrs = 5, $exename = "whois.exe", $whoisexeargs = "-nobanner")
{
    ...
    else
    {
        ...
        Do
        {
            ...
            $pinfo.FileName = $whoisexe
            $pinfo.RedirectStandardError = $true
            $pinfo.RedirectStandardOutput = $true
            $pinfo.UseShellExecute = $false
            $pinfo.Arguments = "-v $($domain) -nobanner"
            ##
            ...
            if ($stderr -ne "")
            {
                Write-Host "DEBUG: $($domain) Error $($stderr)"
            }
            $whoisexe_output = $stdout
            $whoisexe_output = $whoisexe_output.where{$_ -ne ""} # remove blanks
            ...
            #>
                ...
                if ($whoisexe_output -match "Rate limit exceeded")
                {
                    Write-Host "Rate limit exceeded. Pausing 5s..." -ForegroundColor Green
                    Start-Sleep 5
                    $Keeptrying = $true
                }
                else
    ...
    ## whois values
    $whoisvars = New-Object System.Collections.Generic.List[System.Object]
    foreach ($line in $whoisexe_output)
    { # look for lines with:
        $delim_pos = $line.IndexOf(":")
        if ($delim_pos -ne -1)
        {
            $whoisvars.Add($line.Substring(0,$delim_pos+1))
        }
    } # look for lines with:
    ## whoisvlues
    ...
    { #loop through lines
        $line= $line.Trim()
        if (($whoisvars | %{$line.startswith($_)}) -contains $true)
        { #line is of interest
            # Write-Host $line
            $linehash = $line.split(":")
